    ...
    <div className="hidden px-20 mt-32 lg:flex gap-x-2 items-center">
      {breadCrumbs.map((data, index) => {
        return (
          <>
            <span key={index}>
              <Link to={data == 'home' ? '/' : !data?.toLowerCase().includes(title?.toLowerCase()) && `/products/category/${data}`} className="text-sm text-green-600">
                {data?.length > 20 ? `${data?.replace('_', ' ').slice(0, 30)}....` : data?.replace('_', ' ')}
              </Link>
            </span>
          </>
        );
